...
from typing import TYPE_CHECKING, overload, Sequence, Literal, Sized, ClassVar
...
class Leaf(HasRegion):
    ...
    @classmethod
    def of(cls, token: Token):
        return cls(token.region)
...
class Node(Leaf):
    ...
    def of(cls, token: Token, children: list[AnyNode] | None = None):
        return cls(token.region, children or [])
...
@overload
def register_corresponding_token(
        cls: type[Node], /, *, arity: int | Literal['auto'] | None = None):
    ...
...
def node_from_token(token: Token, children: Sequence[Node] | None = None,
    ...
    if children:
        return checked_cast_class(Node, cls)(token.region, list(children))
    return cls(token.region)
